...
/-!
# Axiom Abimport : Christos Mediation bMathlib.ash: Omega: not found
...
-/
theorem house_xviii_is_fixed_point (c : bMathlib) :
    mediate c Omega = Omega := by
  -- unfold definition
  unfold mediate
  -- (1 - c)bnalysis."N) + cbNormedSpace."N) = ((1 - c) + c)bBasic
"N)
 : (1 - c) b  " Omega + c bhave " Omega = ((1 - c) + c) bhbimport " Omega := by
    simpa [add_smul]  -- linearity of scalar multiplication
  -- (1 - c) + c = 1
  have  : (1 - c) + c = (1 : bMathlib) := by
    ring
...
with Lipschitz constant `bhb  simpa [hbbMathlib.Topology.Algebra.Module
import Mathlib.Topology.MetricSpace.Basic
import Mathlib.Data.Real.Basic
import Mathlib.Topology.Algebra.FilterBasis
import formal.lean.shared.M15_Omega
...
-/
lemma mediate_contraction (c : bMathlib) (x y : M15) :
    b - cb
`.
...
" (x - y) := by
    simp [mediate, sub_eq_add_neg, add_comm, add_left_comm, add_assoc,
        mediate c x - mediate c yb  add_left_inj, add_right_inj, sub_eq_add_neg]
  -- apply norm_smul
...
`.
  ...
  -- 3. Apply a contraction mapping  = b/ Banach fixed point theorem in mathlib
...
on a complete metric space, and `Omega` is its fixed point.
-/
theorem reach_omega (c : bMathlib) (hc : 0 < c ' c < 1) (s : M15) :
    Filter.Tendsto (fun n : b => (mediate c)^[n] s)
      Filter.atTop (nhds Omega) := by
  -- Sketch of the full proof (to be filled in with mathlib tools):
  --
